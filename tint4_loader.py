...
class TINT4ModelLoader:
    ...
    def load_model(self, unet_name, model_type):
        ...
        log.info(
            ...
            f"  ({len(_chosen)} weight placeholders)"
        )
        del _quant_specs, _specs_sorted, _seen, _chosen; gc.collect()
        ...
        try:
            ...
            from comfy_aimdo import pin_weight, unpin_weight
        except ImportError:
            pass

        def _aimdo_active():
            if _aimdo_ctrl is None:
                return False
            return getattr(_aimdo_ctrl, '_dynamic_vram_enabled', False)

        def _unpin_tint4(m):
            if not isinstance(m, TINT4Linear):
                return
            m.release_xpu()
        ...
        for module in dm.modules():
            ...
            def _pre(_, inp, mod=m):
                if _aimdo_active():
                    pin_weight(mod)

            def _post(_, inp, out, mod=m):
                if _aimdo_active():
                    unpin_weight(mod)
                    _unpin_tint4(mod)
                return out
