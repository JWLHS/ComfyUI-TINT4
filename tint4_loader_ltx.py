"""
tint4_loader_ltx.py — TINT4 LTX2 Loader v1.0

Fused loader + built-in chunking for LTX2 INT4 models.
Based on verified tint4_loader_video.py + tint4_chunk.py combo.

  - _qt NOT cached — rebuilt from CPU qdata/scale/zp each forward,
    then immediately deleted.  Eliminates persistent GPU qdata copies.
  - Empty-cache every 100 layers to constrain XPU allocator growth.
  - _vgpu (LoRA GPU cache) still cached — first use upload, then reuse.
  - Built-in in-place chunked FFN — no external node needed.
"""
import torch
import torch.nn as nn
import torch.nn.functional as F
import gc, os, json, hashlib, math, logging, types
import folder_paths
import comfy.sd
...
log = logging.getLogger("TINT4-LTX-Loader")
_orig_detect = comfy.model_detection.detect_unet_config


# ═══════════════════════════════════════════════════════════════
# TINT4LinearLTX  (matches verified no-cache version exactly)
# ═══════════════════════════════════════════════════════════════

class TINT4LinearLTX(TINT4Linear):
    ...
    def forward(self, x):
        ...
        # ── Build qt on-the-fly — never cached ──────────────
...
def _build_tint4_lora_index(dm):
    index = {}
...
# ═══════════════════════════════════════════════════════════════
# TINT4 LTX2 Loader  (built-in chunk)
# ═══════════════════════════════════════════════════════════════

class TINT4LTX2Loader:

    NAME = "TINT4 LTX2 Loader"
    CATEGORY = "TINT4"
    FUNCTION = "load_model"
    ...
    def INPUT_TYPES(cls):
        ...
        return {
            "required": {
                "unet_name": (files, {"tooltip": "INT4 LTX2 model"}),
                "chunk": ("INT", {
                    "default": 3, "min": 1, "max": 8,
                    "tooltip": "1 = off.  3 = ~8 GB VRAM on 16 GB Arc.",
                }),
            },
        }
    ...
    RETURN_TYPES = ("MODEL",)
    RETURN_NAMES = ("model",)
    DESCRIPTION = "Load LTX2 INT4 model with built-in FFN chunking."

    def load_model(self, unet_name, chunk):
        unet_path = folder_paths.get_full_path("diffusion_models", unet_name)
        if unet_path is None:
            raise FileNotFoundError(f"'{unet_name}' not found")
        log.info(f"[TINT4 LTX] Loading: {unet_name}")
        ...
        log.info(f"[TINT4 LTX] QuaRot={'ON' if is_quarot else 'OFF'}  "
        ...
        for name, module in dm.named_modules():
            ...
            count += 1
        log.info(f"[TINT4 LTX] Injected {count} TINT4LinearLTX layers")
        ...
        index = _build_tint4_lora_index(dm)
        object.__setattr__(dm, '_tint4_lora_index', index)
        log.info(f"[TINT4 LTX] LoRA index: {len(index)} entries "
                 f"(QuaRot={'ON' if is_quarot else 'OFF'})")

        # ── Built-in chunk (identical logic to verified tint4_chunk.py LTX path) ──
        if chunk > 1:
            blocks = list(getattr(dm, "transformer_blocks", []))
            num_chunks = chunk

            for blk in blocks:
                ff_mod = getattr(blk, "ff", None)
                if ff_mod is None:
                    continue

                def _make_ltx2_ffn_chunked(nc):
                ...
                ff_mod.forward = types.MethodType(_make_ltx2_ffn_chunked(num_chunks), ff_mod)

            log.info(f"[TINT4 LTX] Chunk: {num_chunks} splits, {len(blocks)} blocks patched")
        ...
        def _ltx_detach(unpatch_all=True):
        ...
        log.info(f"[TINT4 LTX] Loaded '{unet_name}'")
        return (model,)


NODE_CLASS_MAPPINGS = {"TINT4LTX2Loader": TINT4LTX2Loader}
NODE_DISPLAY_NAME_MAPPINGS = {"TINT4LTX2Loader": "TINT4 LTX2 Loader"}
