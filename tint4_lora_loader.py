...
class TINT4LoRALoader:
    ...
    def IS_CHANGED(cls, model, lora_name, strength):
        ...
        current = (lora_name, strength, mid)
        if abs(strength) < 1e-5:
            log.info(
                f"[TINT4 LoRA] {lora_name} | strength=0 | disabled | "
                f"{'→ clear' if cls._prev_changed != current else '→ skip'}"
            )
            ...
            log.info(
                f"[TINT4 LoRA] {lora_name} | strength={strength} | unchanged → skip"
            )
        else:
            log.info(
                f"[TINT4 LoRA] {lora_name} | strength={strength} | changed → reload"
            )
